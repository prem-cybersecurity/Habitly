...
let syncDirty = { habits: false, goals: false, events: false, reminders: false, profile: false, settings: false, activityHistory: false, streakRecovery: false };
...
let cloudNamespaceCreatedThisLogin = false;
...
function mergeState(saved, options = {}) {
  ...
  const merged = {
    ...
    streakRecovery: saved.streakRecovery && typeof saved.streakRecovery === 'object' ? clone(saved.streakRecovery) : clone(defaultState.streakRecovery),
...
function habitPriorityValue(h) { return h?.priority === 'High' ? 3 : h?.priority === 'Low' ? 1 : 2; }
function sortHabitsForDisplay(list) {
  return [...list].sort((a,b) => {
    const aDone = pct(a) >= 100 ? 1 : 0, bDone = pct(b) >= 100 ? 1 : 0;
    if (aDone !== bDone) return aDone - bDone; // unfinished first; completed moves down
    const priorityDiff = habitPriorityValue(b) - habitPriorityValue(a);
    if (priorityDiff) return priorityDiff; // High → Medium → Low
    const progressDiff = pct(a) - pct(b);
    if (progressDiff) return progressDiff; // within a priority, least complete first
...
function normalizeState(s) {
  ...
  out.streakRecovery = out.streakRecovery && typeof out.streakRecovery === 'object' ? out.streakRecovery : {};
  out.streakRecovery.thunders = Math.max(0, Math.min(2, Number(out.streakRecovery.thunders ?? 2) || 0));
  out.streakRecovery.rewardedPerfectDays = Math.max(0, Number(out.streakRecovery.rewardedPerfectDays) || 0);
  out.streakRecovery.restoredDates = out.streakRecovery.restoredDates && typeof out.streakRecovery.restoredDates === 'object' ? out.streakRecovery.restoredDates : {};
  ...
  out.syncMeta.mutations = Array.isArray(out.syncMeta.mutations) ? out.syncMeta.mutations.filter(m => m && typeof m === 'object' && ['habits','goals','events','reminders','profile','settings','activityHistory','streakRecovery'].includes(m.kind) && ['upsert','delete','replace'].includes(m.op) && m.at) : [];
  ...
  out.habits = out.habits.map(h => {
    ...
    habit.priority = ['High','Medium','Low'].includes(habit.priority) ? habit.priority : 'Medium';
    return habit;
  });
...
function markSyncDirty(previous, current) {
  ...
  const keys = ['habits','goals','events','reminders','profile','settings','activityHistory','streakRecovery'];
  for (const key of keys) {
    if (JSON.stringify(previous[key] ?? null) !== JSON.stringify(current[key] ?? null)) syncDirty[key] = true;
  }
}
function dirtyFromMutations() {
  const dirty = { habits:false, goals:false, events:false, reminders:false, profile:false, settings:false, activityHistory:false, streakRecovery:false };
...
function appendSyncMutations(previous, current) {
  ...
  for (const kind of ['profile','settings','streakRecovery']) {
...
function applyMutationJournal(remoteState, localState, mutations, baseState = null) {
  ...
  for (const m of [...(mutations || [])]) {
    ...
    } else if (m.kind === 'profile' || m.kind === 'settings' || m.kind === 'streakRecovery') {
...
function touchChangedRecords(previous, current) {
  ...
  for (const key of ['profile', 'settings', 'streakRecovery']) {
...
async function reconcileCloudDocument() {
  ...
  if (!remote) {
    cloudNamespaceCreatedThisLogin = true;
...
async function initializeCloudSync() {
  ...
  cloudNamespaceCreatedThisLogin = false;
  try {
...
function bindReminderControls(root = document) {
  ...
  root.querySelectorAll('[data-edit-event]').forEach(b => b.addEventListener('click', () => eventForm(b.dataset.editEvent))); root.querySelectorAll('[data-thunder-restore]').forEach(b => b.addEventListener('click', () => openThunderEditor(b.dataset.thunderRestore)));
...
function refreshThunderUI(){
  const markup = thunderStatusMarkup();
  document.querySelectorAll('[data-thunder-status]').forEach(el => { el.innerHTML = markup; });
  document.querySelectorAll('[data-thunder-count]').forEach(el => { el.textContent = String(Math.max(0, Math.min(2, Number(state.streakRecovery?.thunders) || 0))); });
}
function thunderIcon(){ return '<svg class="thunder-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M13.2 2 4.8 13h5.7L9.7 22 19.2 9.5h-5.6L13.2 2Z"/></svg>'; }
function topActions() { return `<div class="top-actions"><span data-thunder-status>${thunderStatusMarkup()}</span><span class="sync-pill ${cloudSyncError?'error':''}">${cloudSyncError?'⚠ Sync issue':cloudSyncPending?'↻ Syncing…':'✓ Synced'}</span><button class="action-icon reminder" aria-label="Reminders" data-reminders>${icon('bell')}<span class="badge">${eventNotificationCount()}</span></button><div class="date">${icon('calendar')}<span class="today-label"></span><span class="today-time"></span></div><button class="profile-mini" data-account-switch aria-label="Switch account" title="Switch account">${avatarMarkup()}</button><button class="profile-chevron" data-account-switch aria-label="Switch account" title="Switch account">${icon('chevron')}</button></div>`; }
...
function thunderWindowStartISO() { const d=new Date(todayISO()+'T12:00:00'); d.setDate(d.getDate()-4); return dateISO(d); }
function isThunderEligibleDate(iso) { const t=todayISO(); return iso < t && iso >= thunderWindowStartISO(); }
function dayIsPerfect(iso) { const snap=state.activityHistory?.[iso]; if(!snap?.habits?.length) return false; const active=snap.habits.filter(h=>!h.paused && habitScheduledOnDate(h,iso)); return active.length>0 && active.every(h=>Number(h.percent)>=100); }
function currentNaturalPerfectRun() {
  let run=0,d=new Date(todayISO()+'T12:00:00');
  const restored=state.streakRecovery?.restoredDates||{};
  for(let i=0;i<366;i++){
    const iso=dateISO(d);
    // A Thunder-restored day can reconnect a habit streak, but it must never
    // contribute toward earning another Thunder. It is therefore a hard
    // boundary for the natural 7-day reward run.
    if(restored[iso]) break;
    if(!dayIsPerfect(iso)) break;
    run++;
    d.setDate(d.getDate()-1);
  }
  return run;
}
function refreshThunderRewards() {
  state.streakRecovery=state.streakRecovery||{thunders:2,rewardedPerfectDays:0,restoredDates:{}};
  const run=currentNaturalPerfectRun();
  // Reward progress belongs only to the uninterrupted natural-perfect run
  // since the most recent Thunder restoration.
  if(run < state.streakRecovery.rewardedPerfectDays) state.streakRecovery.rewardedPerfectDays=0;
  while(run >= state.streakRecovery.rewardedPerfectDays + 7){
    state.streakRecovery.rewardedPerfectDays += 7;
    if(state.streakRecovery.thunders < 2) state.streakRecovery.thunders += 1;
  }
  state.streakRecovery.thunders=Math.max(0,Math.min(2,state.streakRecovery.thunders));
}
function thunderStatusMarkup() { const count=Math.max(0,Math.min(2,Number(state.streakRecovery?.thunders)||0)); return `<span class="thunder-balance" title="Streak Restore Thunders" aria-label="${count} Thunder available">${thunderIcon()}<b>${count}</b><small>/2 Thunder</small></span>`; }
function historicalHabitValue(h, iso) { const snap=state.activityHistory?.[iso]; const rec=snap?.habits?.find(x=>x.id===h.id); return rec ? Math.max(0, Number(rec.current)||0) : Math.max(0, Number((h.daily||{})[iso]||0)); }
function openThunderEditor(iso) { if(!isThunderEligibleDate(iso)) return; const recovery=state.streakRecovery||{}; if(Number(recovery.thunders||0)<1){ toast('No Thunder available'); return; } if(recovery.restoredDates?.[iso]){ toast('This day has already been restored'); return; } const dateLabel=formatDate(iso); const habits=state.habits.filter(h=>!h.paused && habitScheduledOnDate(h,iso)); if(!habits.length){ toast('No scheduled habits on this day'); return; } modal(`Restore ${dateLabel}`, 'Use 1 ⚡ Thunder to enter the real values you completed on this day.', `<form class="form" id="thunderRestoreForm"><div class="thunder-restore-banner"><span>⚡</span><div><b>${esc(recovery.thunders)} Thunder${Number(recovery.thunders)===1?'':'s'} available</b><small>This does not make the day 100%. Your real values determine each habit's streak.</small></div></div><div class="thunder-habit-editor">${habits.map(h=>{const v=historicalHabitValue(h,iso); const target=h.type==='yesno'?1:Math.max(1,Number(h.target)||1); return `<label class="field"><span>${esc(h.emoji||'🌱')} ${esc(h.name)} <small>Target ${esc(target)} ${esc(h.unit||'times')}</small></span><input type="number" name="habit_${esc(h.id)}" min="0" max="${esc(target)}" step="any" value="${esc(v)}"></label>`;}).join('')}</div><div class="form-actions"><button type="button" class="secondary-btn" data-modal-close>Cancel</button><button class="primary-btn">Restore with ⚡</button></div></form>`); const form=document.getElementById('thunderRestoreForm'); form?.addEventListener('submit',e=>{e.preventDefault(); const now=new Date().toISOString(); const existing=state.activityHistory?.[iso]; const snap=existing?clone(existing):buildSnapshot(state,iso); snap.date=iso; snap.habits=habits.map(h=>{const target=h.type==='yesno'?1:Math.max(1,Number(h.target)||1); const raw=Number(new FormData(form).get(`habit_${h.id}`)); const current=Math.max(0,Math.min(target,Number.isFinite(raw)?raw:0)); h.daily=h.daily||{}; h.daily[iso]=current; h.dailyUpdatedAt=h.dailyUpdatedAt||{}; h.dailyUpdatedAt[iso]=now; return { id:h.id,name:h.name,emoji:h.emoji||'',category:h.category,paused:!!h.paused,frequency:h.frequency||'Daily',selectedDays:Array.isArray(h.selectedDays)?h.selectedDays.slice():[],target,unit:h.unit||'',current,percent:Math.min(100,Math.round(current/target*100))}; }); state.activityHistory=state.activityHistory||{}; state.activityHistory[iso]=snap; state.streakRecovery=state.streakRecovery||{thunders:2,rewardedPerfectDays:0,restoredDates:{}}; state.streakRecovery.thunders=Math.max(0,Number(state.streakRecovery.thunders||0)-1); state.streakRecovery.restoredDates={...(state.streakRecovery.restoredDates||{}),[iso]:now}; habitStreakCache.clear(); save(); closeModal(); render(); toast(`Streak history restored for ${dateLabel} ⚡`); }); }
const habitStreakCache = new Map();
...
function celebrateHabitCompletion(habitId) {
  const habit = state.habits.find(x => x.id === habitId);
  if (!habit || document.querySelector('.habit-completion-popup')) return;
  const popup = document.createElement('div');
  popup.className = 'habit-completion-popup';
  popup.setAttribute('role', 'status');
  popup.setAttribute('aria-live', 'polite');
  popup.innerHTML = `
    <div class="habit-completion-orb">
      <span class="habit-completion-ring ring-one"></span>
      <span class="habit-completion-ring ring-two"></span>
      <span class="habit-completion-logo"><img src="${AS}Habitly Leaf Transparent.png" alt="Habitly"></span>
    </div>
    <strong>Habit Complete</strong>
    <small>${esc(habit.name)}</small>`;
  document.body.appendChild(popup);
  requestAnimationFrame(() => popup.classList.add('show'));
  window.setTimeout(() => {
    popup.classList.remove('show');
    window.setTimeout(() => popup.remove(), 360);
  }, 1450);
}
...
function habitRow(h, opts = {}) {
  ...
  return `<article class="habit-row ${compact ? 'compact' : ''} ${h.paused ? 'is-paused' : ''}" data-id="${h.id}" data-category="${esc(h.category)}">
    ...
    <div class="habit-name"><strong>${esc(h.name)}</strong>${streak > 0 ? `<small class="habit-streak">🔥 ${streak}d streak</small>` : ''}</div>
...
function renderHabitCards(filter = 'all', sort = 'priority') {
  ...
  if (sort === 'name') list.sort((a, b) => completionFirst(a,b) || a.name.localeCompare(b.name) || (habitPriorityValue(b) - habitPriorityValue(a)));
  if (sort === 'recent') list.sort((a, b) => completionFirst(a,b) || ((b.created || 0) - (a.created || 0)) || (habitPriorityValue(b) - habitPriorityValue(a)));
...
function initStatistics() {
  ...
  function updateInsights() {
    ...
    if (!target) return;
    const dates = period === 'day' ? [todayISO()] : period === 'week' ? weeklyActivityData(new Date()).filter(x => x.value !== null).map(x => x.date) : period === 'month' ? dateRange(new Date().getFullYear() + '-' + String(new Date().getMonth() + 1).padStart(2, '0') + '-01', todayISO()).map(dateISO) : dateRange(custom.s, custom.e).map(dateISO);
    const history = dates.map(d => state.activityHistory?.[d]).filter(Boolean);
    const byHabit = new Map();
    history.forEach(snap => (snap.habits || []).filter(h => !h.paused).forEach(h => {
      const key = h.id || h.name;
      const item = byHabit.get(key) || { name: h.name, emoji: h.emoji || '', values: [] };
      item.values.push(Number(h.percent) || 0);
      byHabit.set(key, item);
    }));
    const habitStats = [...byHabit.values()].map(x => ({ ...x, avg: x.values.length ? Math.round(x.values.reduce((a,v)=>a+v,0)/x.values.length) : 0 })).sort((a,b)=>b.avg-a.avg);
    const best = habitStats[0];
    const needs = habitStats.length ? [...habitStats].sort((a,b)=>a.avg-b.avg)[0] : null;
    const dayStats = dates.map(d => ({ date:d, value: state.activityHistory?.[d] ? dailyProgress(d) : null })).filter(x=>x.value!==null);
    const bestDay = dayStats.length ? [...dayStats].sort((a,b)=>b.value-a.value)[0] : null;
    const avg = dayStats.length ? Math.round(dayStats.reduce((a,x)=>a+x.value,0)/dayStats.length) : 0;
    const cards = [];
    if (best) cards.push(`<article class="insight-card"><span class="insight-icon">🏆</span><div><small>Most consistent habit</small><strong>${esc(best.emoji)} ${esc(best.name)}</strong><span>${best.avg}% average completion</span></div></article>`);
    if (needs && (!best || needs.name !== best.name)) cards.push(`<article class="insight-card"><span class="insight-icon">🌱</span><div><small>Needs attention</small><strong>${esc(needs.emoji)} ${esc(needs.name)}</strong><span>${needs.avg}% average completion</span></div></article>`);
    if (bestDay) cards.push(`<article class="insight-card"><span class="insight-icon">📅</span><div><small>Best recorded day</small><strong>${esc(formatDate(bestDay.date))}</strong><span>${bestDay.value}% daily completion</span></div></article>`);
    cards.push(`<article class="insight-card"><span class="insight-icon">↗</span><div><small>Recorded consistency</small><strong>${avg}%</strong><span>${dayStats.length ? `${dayStats.length} recorded day${dayStats.length===1?'':'s'}` : 'No recorded activity yet'}</span></div></article>`);
...
function bindHabitInteractions(root) {
  root.querySelectorAll('[data-habit-action]').forEach(b => {
    ...
    const applyAction = (action, amount = 1, shouldRender = true) => {
      ...
      const beforePercent = habitPctForDate(h, todayISO());
...
function calendarHabitsForDay(d) {
  ...
  if (!snap) return state.habits.filter(h => !h.paused && habitScheduledOnDate(h, iso)).map(h => ({ habit: h, percent: 0, current: 0, target: h.target, unit: h.unit }));
...
function monthView(cursor) {
  ...
  for (let n = 1; n <= days; n++) {
    ...
    out += `<button class="cal-day ${sel ? 'selected' : ''} ${today ? 'today' : ''} ${p >= 100 ? 'is-complete' : ''} ${p === 0 ? 'is-empty' : ''}" data-cal-date="${iso}"><span class="day-number">${n}</span>${calendarRingMarkup(p)}<span class="day-percent">${p ? p + '%' : ''}</span>${(state.streakRecovery?.restoredDates||{})[iso] ? `<span class="thunder-day-badge" title="Streak history restored">${thunderIcon()}</span>` : ''}</button>`;
    cells++;
  }
...
function weekView(cursor) {
  ...
  names.slice(offset).concat(names.slice(0, offset)).forEach((n, i) => {
    ...
    out += `<button class="week-day ${sel ? 'selected' : ''} ${p >= 100 ? 'is-complete' : ''} ${p === 0 ? 'is-empty' : ''}" data-cal-date="${iso}"><strong>${n}</strong><span class="day-number">${d.getDate()}</span>${calendarRingMarkup(p, 'week-ring')}<span class="week-percent">${p ? p + '%' : ''}</span>${(state.streakRecovery?.restoredDates||{})[iso] ? `<span class="thunder-day-badge week-thunder-badge" title="Streak history restored">${thunderIcon()}</span>` : ''}<div class="progress-track"><i style="width:${p}%"></i></div></button>`;
  });
  return out + '</div>';
}
...
function settingsSection(key) {
 ...
 <div class="settings-two-col"><article class="settings-card"><div class="settings-card-head compact"><div class="settings-heading-icon green">${icon('shield')}</div><div><h2>Account status</h2><p>Your Habitly profile is ready to use.</p></div><span class="status-pill">Good</span></div><div class="info-list"><div><span>${icon('check')}Profile information</span><b>Complete</b></div><div><span>${icon('database')}Local data</span><b>Protected</b></div></div></article><article class="settings-card about-mini"><div class="settings-brand-mark">${logo()}</div><h2>Habitly by PRK</h2><p>Build better habits. Achieve your goals. One consistent day at a time.</p></article></div>`;
  ...
  if (key === 'help') return `<article class="settings-card help-settings-card"><div class="settings-card-head"><div class="settings-heading-icon purple">${icon('info')}</div><div><h2>Help & Guide</h2><p>A simple guide to help you get the most from Habitly.</p></div></div><div class="help-intro"><div class="help-intro-icon"><img src="assets/Habitly Leaf Transparent.png" alt="Habitly leaf logo"></div><div><h3>Getting started</h3><p>Habitly is built around three simple ideas: create habits, complete them consistently, and use your progress to reach bigger goals.</p></div></div><div class="help-guide-grid"><article class="help-guide-step"><span>01</span><div><h3>Create a habit</h3><p>Give it a name, choose a target, select the days you want to follow it, and set a priority.</p></div></article><article class="help-guide-step"><span>02</span><div><h3>Track it every day</h3><p>Use the − and + controls to update quantity habits. A scheduled day counts as complete only when the habit reaches 100%.</p></div></article><article class="help-guide-step"><span>03</span><div><h3>Build your streak</h3><p>Your streak grows from fully completed scheduled days. Partial progress does not extend the streak.</p></div></article><article class="help-guide-step"><span>04</span><div><h3>Use Goals</h3><p>Use goals for bigger outcomes, milestones and supporting habits. Fitness goals can track your progress separately.</p></div></article><article class="help-guide-step"><span>05</span><div><h3>Review Calendar & Statistics</h3><p>Calendar shows your daily history, while Statistics helps you understand consistency and trends.</p></div></article><article class="help-guide-step"><span>06</span><div><h3>Keep your data synced</h3><p>Your signed-in Habitly account uses cloud sync across devices. Google Drive is optional for backup and restore.</p></div></article></div><div class="help-faq"><h3>Quick answers</h3><details><summary>Why didn't my streak increase?</summary><p>The habit must reach 100% on a scheduled day. For example, 80% is useful progress, but it does not complete that day.</p></details><details><summary>Why did a habit move lower in the list?</summary><p>Habitly places unfinished habits above completed habits. Priority is used to arrange unfinished habits without adding extra labels to every card.</p></details><details><summary>Can I change my habit schedule later?</summary><p>Yes. Edit the habit and update its selected days or target without creating a new habit.</p></details></div><div class="help-contact-card"><div><b>Need more help?</b><small>Contact, report an issue, or share an opinion through the Habitly contact page.</small></div><a class="primary-btn" href="https://premkumar-portfolio-kohl.vercel.app/#contact" target="_blank" rel="noopener noreferrer">Contact / Feedback ${icon('external')}</a></div></article>`;
  return `<article class="settings-card about-settings"><div class="about-hero"><div class="about-logo"><img src="${AS}Habitly Leaf Transparent.png" alt="Habitly leaf logo"></div><div><span class="eyebrow">HABITLY BY PRK</span><h2>About Habitly</h2><p>Your personal habit and goal tracking companion.</p></div></div><div class="about-copy"><h3>About me</h3><p>I'm Prem Kumar, an Integrated B.Tech–M.Tech Cyber Security student focused on cloud security, secure software and practical cybersecurity engineering. I build hands-on projects to strengthen my skills in secure systems, automation and real-world application development.</p><h3>Why I created Habitly</h3><p>I created Habitly as a practical productivity application that brings habits and long-term goals into one focused workspace. It makes progress visible, measurable and editable while giving me a real-world project for building responsive interfaces, state management, persistence and user-focused software.</p></div><div class="about-links"><a href="https://github.com/prem-cybersecurity" target="_blank" rel="noopener noreferrer" aria-label="Open GitHub">${icon('github')}<span>GitHub</span></a><a href="https://premkumar-portfolio-kohl.vercel.app/#contact" target="_blank" rel="noopener noreferrer" aria-label="Open portfolio contact"><span>Portfolio</span>${icon('external')}</a><a class="linkedin-link" href="https://www.linkedin.com/in/premkumar-cybersecurity" target="_blank" rel="noopener noreferrer" aria-label="Open LinkedIn">${icon('linkedin')}<span>LinkedIn</span></a></div><small class="version-line">Habitly by PRK · Version ${APP_VERSION}</small></article>`;
}
...
function habitForm(id) {
  ...
  modal(id ? 'Edit habit' : 'Add habit', id ? 'Refine your routine without losing progress.' : 'Create a routine you can actually maintain.', `<form class="form" id="habitForm">
    ...
    <div class="form-grid two"><div class="field"><label>Priority</label><select name="priority"><option value="High" ${h?.priority === 'High' ? 'selected' : ''}>High</option><option value="Medium" ${!h?.priority || h?.priority === 'Medium' ? 'selected' : ''}>Medium</option><option value="Low" ${h?.priority === 'Low' ? 'selected' : ''}>Low</option></select><small>Higher-priority unfinished habits appear first.</small></div><div></div></div>
    ...
    <section class="habit-live-preview" id="habitLivePreview"><div class="preview-label">Live preview</div><div class="preview-habit-row"><span class="preview-emoji" id="previewEmoji">${esc(h?.emoji || '🌱')}</span><div><strong id="previewName">${esc(h?.name || 'Your habit')}</strong><small id="previewSchedule">Daily · Medium priority</small></div><span class="preview-target" id="previewTarget">${h?.target ? `${esc(h.target)} ${esc(h.unit || 'times')}` : 'Set a target'}</span></div></section>
  ...
  const preview = form.querySelector('#habitLivePreview');
  const updateHabitPreview = () => {
    if (!preview) return;
    const name = String(form.querySelector('[name="name"]')?.value || '').trim() || 'Your habit';
    const emoji = String(form.querySelector('[name="emoji"]')?.value || '').trim() || '🌱';
    const target = String(form.querySelector('[name="target"]')?.value || '').trim();
    const unit = String(form.querySelector('[name="unit"]')?.value || '').trim() || 'times';
    const priority = String(form.querySelector('[name="priority"]')?.value || 'Medium');
    const frequency = String(frequencyInput?.value || 'Daily');
    const days = [...form.querySelectorAll('[name="selectedDay"]:checked')].map(x=>x.nextElementSibling?.textContent || '').filter(Boolean);
    const schedule = frequency === 'Selected Days' ? (days.length ? days.join(' · ') : 'Choose your days') : 'Every day';
    form.querySelector('#previewName').textContent = name;
    form.querySelector('#previewEmoji').textContent = emoji;
    form.querySelector('#previewSchedule').textContent = `${schedule} · ${priority} priority`;
    const trackType = form.querySelector('[name="type"]:checked')?.value || 'quantity';
    form.querySelector('#previewTarget').textContent = trackType === 'yesno' ? 'Yes / No' : (target ? `${target} ${unit}` : 'Set a target');
  };
  form.querySelectorAll('input,select').forEach(el => el.addEventListener('input', updateHabitPreview));
  form.querySelectorAll('input,select').forEach(el => el.addEventListener('change', updateHabitPreview));
  updateHabitPreview();
  ...
  form.addEventListener('submit', e => {
    ...
    const priority = ['High','Medium','Low'].includes(String(fd.get('priority') || '')) ? String(fd.get('priority')) : 'Medium';
    ...
    if (id) {
      ...
      Object.assign(h, { name: String(fd.get('name')).trim(), category: fd.get('category'), emoji: String(fd.get('emoji') || '').trim(), type, target, unit, priority, frequency, selectedDays: frequency==='Selected Days'?selectedDays:[0,1,2,3,4,5,6], paused: fd.get('paused') === 'on', updatedAt: new Date().toISOString() });
      ...
      state.habits.push({ id: uid('h'), name: String(fd.get('name')).trim(), emoji: String(fd.get('emoji') || '').trim(), category: fd.get('category'), type, target, current: 0, unit, priority, paused: false, created: Date.now(), frequency, selectedDays: frequency==='Selected Days'?selectedDays:[0,1,2,3,4,5,6], updatedAt: new Date().toISOString() });
    }
...
function habitlyIsStandalone() { return !!(window.matchMedia?.('(display-mode: standalone)').matches || window.navigator.standalone === true); }
function showNewUserGuide() {
  if (!currentAuthUser?.id) return;
  const key = `habitly.guide.seen.${currentAuthUser.id}`;
  if (localStorage.getItem(key) === '1') { showNewUserInstallWelcome(); return; }
  const markGuideSeen = () => localStorage.setItem(key, '1');
  setTimeout(() => {
    if (!currentAuthUser || currentRoute() === 'login') return;
    const steps = [
      { title:'Create your first habit', text:'Choose what you want to improve, set a target, choose your schedule and set a priority.', icon:'target' },
      { title:'Track progress to 100%', text:'Use the − and + buttons to update your progress. A scheduled day is complete only when the habit reaches 100%.', icon:'check' },
      { title:'Build your streak 🔥', text:'Your streak increases only for fully completed scheduled days. Partial progress does not extend the streak.', icon:'star' },
      { title:'Use Goals 🎯', text:'Goals are for bigger outcomes and milestones. Supporting habits can help you reach them.', icon:'trophy' },
      { title:'Review Calendar & Statistics', text:'Use Calendar for daily activity and Statistics for consistency, trends and your heatmap.', icon:'calendar' },
      { title:'Keep everything in sync ☁️', text:'Your authenticated account uses cloud sync across devices. Google Drive is available for backup and restore.', icon:'cloud' }
    ];
    let step = 0;
    const renderGuide = () => {
      const item = steps[step];
      const isLast = step === steps.length - 1;
      modal('Welcome to Habitly 🌱', 'A quick guide to getting the most from Habitly.', `<div class="onboarding-guide"><div class="onboarding-logo-wrap"><img src="assets/Habitly Leaf Transparent.png" alt="Habitly"></div><div class="onboarding-step-count">${step + 1} of ${steps.length}</div><h3>${esc(item.title)}</h3><p>${esc(item.text)}</p><div class="onboarding-dots">${steps.map((_,i)=>`<i class="${i===step?'active':''}"></i>`).join('')}</div><div class="onboarding-actions"><button type="button" class="secondary-btn" id="guideSkip">Skip</button><button type="button" class="primary-btn" id="guideNext">${isLast?'Finish':'Next →'}</button></div><small>You can read this guide again anytime from Settings → Help & Guide.</small></div>`);
      document.getElementById('guideSkip')?.addEventListener('click', () => { markGuideSeen(); closeModal(); showNewUserInstallWelcome(); });
      document.getElementById('guideNext')?.addEventListener('click', () => { if (isLast) { markGuideSeen(); const firstHabit = !state.habits.length; closeModal(); if (firstHabit) setTimeout(() => habitForm(), 160); else setTimeout(() => showNewUserInstallWelcome(), 160); } else { step += 1; renderGuide(); } });
    };
    renderGuide();
  }, 500);
}

function showNewUserInstallWelcome() {
  if (!currentAuthUser?.id || habitlyIsStandalone()) return;
  const key = `habitly.install.welcome.${currentAuthUser.id}`;
  if (localStorage.getItem(key) === '1') return;
  localStorage.setItem(key, '1');
  setTimeout(() => {
    if (!currentAuthUser || currentRoute() === 'login') return;
    const action = deferredInstallPrompt ? '<button type="button" class="primary-btn" id="welcomeInstallBtn">Install Habitly</button>' : '<button type="button" class="primary-btn" id="welcomeInstallBtn">How to install</button>';
    modal('Welcome to Habitly 👋', 'Make Habitly part of your daily routine.', `<div class="install-welcome"><div class="install-welcome-icon">${icon('download')}</div><h3>Use Habitly like an app</h3><p>Add Habitly to your device for faster access and a clean app-style experience.</p><div class="install-welcome-actions">${action}<button type="button" class="secondary-btn" data-modal-close>Maybe later</button></div><small>You can always install it later from Settings → Account.</small></div>`);
    document.getElementById('welcomeInstallBtn')?.addEventListener('click', async () => {
      if (deferredInstallPrompt) {
        ...
        try { await prompt.prompt(); await prompt.userChoice; } catch (e) { console.warn('Welcome install prompt failed:', e); }
        closeModal();
        return;
      }
      closeModal();
      const standalone = habitlyIsStandalone();
      if (standalone) return;
      if (/iphone|ipad|ipod/i.test(navigator.userAgent)) toast('Safari: Share → Add to Home Screen');
...
async function handleAuthenticatedUser(nextUser) {
  ...
  initializeCloudSync().finally(() => {
    ...
    if (location.hash !== '#/login') {
      render();
      if (cloudNamespaceCreatedThisLogin) showNewUserGuide();
    }
  });
}
