...
function currentPerfectRun() { let run=0,d=new Date(todayISO()+'T12:00:00'); for(let i=0;i<366;i++){const iso=dateISO(d); if(!dayIsPerfect(iso)) break; run++; d.setDate(d.getDate()-1);} return run; }
function refreshThunderRewards() { state.streakRecovery=state.streakRecovery||{thunders:2,rewardedPerfectDays:0,restoredDates:{}}; const run=currentPerfectRun(); if(run < state.streakRecovery.rewardedPerfectDays) state.streakRecovery.rewardedPerfectDays=0; while(run >= state.streakRecovery.rewardedPerfectDays + 7){ state.streakRecovery.rewardedPerfectDays += 7; if(state.streakRecovery.thunders < 2) state.streakRecovery.thunders += 1; } state.streakRecovery.thunders=Math.max(0,Math.min(2,state.streakRecovery.thunders)); }
function thunderStatusMarkup() { const count=Math.max(0,Math.min(2,Number(state.streakRecovery?.thunders)||0)); return `<span class="thunder-balance" title="Streak Restore Thunders">⚡ ${count}/2</span>`; }
